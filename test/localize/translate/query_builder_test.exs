defmodule Localize.Translate.QueryBuilderTest do
  use Localize.Translate.TestCase

  import Localize.Translate.QueryBuilder

  alias Localize.Translate.{Article, Book, Comment}
  ...
  test "that a valid locale that has no translations returns nil for locale chain" do
    ...
    # Both rows return NULL and count(column) doesn't include
  ...
  test "that a valid locale that has no translations returns nil for dynamic locales" do
    count =
      Repo.one(
        from(
          a in Book,
          where: is_nil(translated(Book, a, Localize.Translate.Factory.locales(:it))),
          select: count(a.id)
        )
      )
    ...
    count =
      Repo.one(
        from(
          a in Book,
          where:
            not is_nil(translated(Book, a0.title, Localize.Translate.Factory.locales([:it, :es]))),
          select: count(a.id)
        )
      )
  ...
  test "should select all books with dynamic fallback chain" do
    result =
      Repo.all(
        from(
          a in Book,
          select: translated_as(Book, a.title, Localize.Translate.Factory.locales([:it, :es])),
          where:
            not is_nil(translated(Book, a.title, Localize.Translate.Factory.locales([:it, :es])))
        )
      )
  ...
  test "model with partial translations inserts" do
    brochure = insert(:brochure)
    assert %Localize.Translate.Brochure{} = brochure
    assert %Localize.Translate.Brochure.Translations{} = brochure.translations
    assert %Localize.Translate.Brochure.Translations.Fields{} = brochure.translations.ja
  end
  ...
  test "that the translated column is a simple string",
    ...
    [first, second] =
      Repo.all(
        from(
          a in Book,
          select: translated(Book, a.title, :en)
        )
      )
  ...
  test "should raise when adding conditions to an untranslatable field" do
    ...
    assert_raise ArgumentError,
                 "'Localize.Translate.Article' module must declare 'translations' as translatable",

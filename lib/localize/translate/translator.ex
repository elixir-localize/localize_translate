defmodule Localize.Translate.Translator do
  @moduledoc """
  Provides easy access to struct translations.

  Although translations are stored in regular fields of a struct and can be accessed directly,
  **it is recommended to access translations using the functions provided by this module**
  instead. These functions present additional behaviours such as:

  * Checking that the given struct uses `Localize.Translate`
  ...
  * Falling back along a locale fallback chain (list of locales in which to look for
    a translation). If not found, then return the default value or raise an exception if a
  ...
  * `locale` is either a single locale or a list of locales (a locale fallback chain). Locales
    are atoms or strings.

  ### Returns

  * The translated struct, falling back to the default value if no translation is available.
  ...
  Assuming the example article in this module, we can translate the entire struct into Spanish:
      ...
      article_es = Localize.Translate.Translator.translate(article, :es)

      article_es.title #=> "Cómo escribir un corrector ortográfico"
      article_es.body #=> "Un artículo maravilloso de Peter Norvig"

  Falls back to the default locale if the translation does not exist:

      # The Deutsch translation does not exist so the default values are returned
      article_de = Localize.Translate.Translator.translate(article, :de)
  ...
  until a translation is found. If none is found, the default value is returned.
      ...
      article_de = Localize.Translate.Translator.translate(article, [:de, :es])

      article_de.title #=> "Cómo escribir un corrector ortográfico"
      article_de.body #=> "Un artículo maravilloso de Peter Norvig"

  """
  @spec translate(Localize.Translate.translatable(), Localize.Translate.locale_list()) ::
          Localize.Translate.translatable()

  @spec translate(Localize.Translate.translatable(), atom, Localize.Translate.locale_list()) ::
          any
  ...
  * `locale` is either a single locale or a list of locales (a locale fallback chain).

  ### Returns

  * The translated value for the field, or the default value if no translation is available.
  ...
  Assuming the example article in this module:
      ...
      Localize.Translate.Translator.translate(article, :fake_attr, :es)
  ...
  def translate(%{__struct__: module} = translatable, field, locale)
    ...
    unless Localize.Translate.translatable?(translatable, field) do
  ...
  * Raises a `RuntimeError` if no translation is available.
